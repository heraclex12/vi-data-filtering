...
class VisualizationForLang:
    def __init__(
        ...
        self.model_lang_id = LoadParameters.load_model_lang_id(path_fasttext_model)
        self.sentencepiece_model = LoadParameters.load_sentencepiece_model(path_sentencepiece_model)
        self.sentencepiece_model_tok = (
            self.sentencepiece_model if self.param["tokenization"] else None
        )
        self.kenlm_model = LoadParameters.load_kenlm_model(path_kenlm_model)
    ...
    def filtering_of_docs(self):
        def set_sliders():
            ...
            if "number_words" in columns:
                with st.sidebar.expander("Number of words"):
                    ...
                    keys.append(new_key)
                    VisualizationForLang.plot_hist(self.docs, new_key)
                    cond_1 = get_cond(new_key[0], new_key[1], new_key[2])
                    VisualizationForLang.print_discarded_by_cond(cond_1)
                    ...
                    VisualizationForLang.print_discarded_by_cond(cond_2)
            ...
            if "character_repetition_ratio" in columns:
                with st.sidebar.expander("Character repetition ratio"):
                    ...
                    VisualizationForLang.print_discarded_by_cond(cond)
        ...
        with st.expander(
            ...
            if self.docs.empty:
                ...
                VisualizationForLang.display_dataset(
        ...
        if not (self.words is None):
            ...
            if "len_word" in columns:
                with st.sidebar.expander("Length of words"):
                    ...
                    VisualizationForLang.plot_hist(self.words, new_key)
                    cond_len_words = self.words["len_word"] <= cutoff_word
                    VisualizationForLang.print_discarded_by_cond(cond_len_words)
            ...
            if "incorrect_substrings" in columns:
                with st.sidebar.expander("Words with incorrect substrings"):
                    ...
                    VisualizationForLang.print_discarded_by_cond(
...
class Visualization:
    ...
    def preamble():
        st.markdown(
            "This visualization is to help you customize the filtering parameters easily through interactively "
            "choosing suitable parameters that optimize reducing trash data.",
            unsafe_allow_html=True,
        )

    def visualize(self):
        visualization_for_lang = VisualizationForLang(
            path_data=self.params["path_data"],
            lang=self.params["lang"],
            num_docs=self.params["num_docs"],
            num_docs_for_words=self.params["num_docs_for_words"],
            max_len_text_display=self.params["max_len_text_display"],
            lang_dataset_id=self.params["lang_dataset_id"],
            path_fasttext_model=self.params["path_fasttext_model"],
            path_sentencepiece_model=self.params["path_sentencepiece_model"],
            path_kenlm_model=self.params["path_kenlm_model"],
        )
...
params = {
    "path_data": f"./visualization/vi_examples_with_stats.json",
    "lang": "vi",
    "num_docs": 15000,
    "num_docs_for_words": 1500,
    "max_len_text_display": 10000,
    "lang_dataset_id": "vi",
    "path_fasttext_model": "./models/lid.176.bin",
    "path_sentencepiece_model": f"./models/vi.sp.model",
    "path_kenlm_model": f"./models/vi.arpa.bin",
}
